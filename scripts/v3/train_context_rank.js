...
function runTrainContextRank() {
  ...
  const pairwiseData = JSON.parse(fs.readFileSync(pairwisePath, 'utf8'));
  ...
  const trainRowsByEpisode = new Map();
  const valRowsByEpisode = new Map();
...
function evaluateModelOnRows(model, rowsByEpisode) {
  const taskEvals = [];
  ...
  for (const [epId, rows] of rowsByEpisode.entries()) {
    const scored = rows.map((r) => {
      ...
      return {
        ...
        tokenEstimate: (r.features && r.features.tokenEstimate) || 100,
      };
    });
...
module.exports = { runTrainContextRank };
