...
function runFrozenV2Baseline(options = {}) {
  ...
  try {
    ...
    let testEpisodes = manifest.episodes.filter((e) => testAssignments.some((a) => a.episodeId === e.episodeId));
...
async function main() {
  ...
  const tasksToRun = JSON.parse(fs.readFileSync(path.join(__dirname, 'test_episodes.json'), 'utf8'));
  console.log(\`Evaluating \${tasksToRun.length} held-out test episodes on frozen V2 baseline...\`);

  // Index repositories
  const indexes = {};
  for (const [repoKey, config] of Object.entries(repoConfigs)) {
    if (fs.existsSync(config.path)) {
      ...
      indexes[repoKey] = { units: idx.units, graph, gitInt, path: config.path };
  ...
  for (let i = 0; i < tasksToRun.length; i++) {
    ...
    const candidates = candGen.generateCandidates(taskCtx, repoData.units, repoData.graph, repoData.gitInt);
    ...
    for (const u of repoData.units) unitsMap.set(u.id, u);

    const featuresList = candidates.map(c => {
      const u = unitsMap.get(c.contextUnitId);
      if (!u) return null;
    ...
    for (const cand of ranked) {
      const unit = unitsMap.get(cand.contextUnitId);
      const tok = unit && unit.metadata && typeof unit.metadata.tokenEstimate === 'number'
        ? unit.metadata.tokenEstimate
        : 100;
  ...
  const mean = (fn) => Number((taskResults.reduce((acc, t) => acc + fn(t), 0) / (n || 1)).toFixed(4));
  ...
  const aggregate = {
    ...
    meanLatencyMs: Number((taskResults.reduce((acc, t) => acc + t.latencyMs, 0) / (n || 1)).toFixed(1)),
    meanContextTokens: Math.round(taskResults.reduce((acc, t) => acc + t.contextTokens, 0) / (n || 1)),
...
module.exports = { runFrozenV2Baseline, FROZEN_V2_SHA, EXPECTED_TAG };
